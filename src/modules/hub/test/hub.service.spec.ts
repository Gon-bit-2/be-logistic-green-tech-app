...
describe('HubService', () => {
  ...
  describe('assignStaff', () => {
    it('gán nhân viên thành công nếu role là WAREHOUSE_STAFF', async () => {
      hubRepo.findById.mockResolvedValue({ id: 1 } as any);
      userRepo.findUniqueIncludeRolePermissions.mockResolvedValue({ id: 99, role: { name: roleName.WAREHOUSE_STAFF } } as any);
    ...
    it('quăng NotFoundException nếu không có user', async () => {
      hubRepo.findById.mockResolvedValue({ id: 1 } as any);
      userRepo.findUniqueIncludeRolePermissions.mockResolvedValue(null);
    ...
    it('quăng BadRequestException nếu role KHÔNG phải WAREHOUSE_STAFF', async () => {
      hubRepo.findById.mockResolvedValue({ id: 1 } as any);
      userRepo.findUniqueIncludeRolePermissions.mockResolvedValue({ id: 99, role: { name: roleName.DRIVER } } as any);
  ...
  describe('removeStaff', () => {
    it('xóa nhân viên thành công nếu hợp quy', async () => {
      hubRepo.findById.mockResolvedValue({ id: 1 } as any);
      userRepo.findUniqueIncludeRolePermissions.mockResolvedValue({ 
        id: 99, 

...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('đăng ký thành công', async () => {
      verificationCodeRepo.findUniqueVerificationCode.mockResolvedValue({
        code: '123456',
      ...
      prismaService.$transaction.mockResolvedValue([{ id: 1, email: 'test@mail.com' }]);

      const res = await service.register({ email: 'test@mail.com', password: 'test', fullName: 'Tester', phone: '0123' });
    ...
    it('văng UnprocessableEntityException do mã OTP không hợp lệ', async () => {
      verificationCodeRepo.findUniqueVerificationCode.mockResolvedValue(null);
      await expect(service.register({ email: 'test@mail.com', password: 'test', fullName: 'Tester', phone: '0123' }))
    ...
    it('văng BadRequestException khi prisma transaction throw error (user existed)', async () => {
      verificationCodeRepo.findUniqueVerificationCode.mockResolvedValue({ expiresAt: addMilliseconds(new Date(), 100000) } as any);
      sharedRoleRepo.getClientRoleId.mockResolvedValue(2);
      prismaService.$transaction.mockRejectedValue(new Error('Unique constraint'));
  ...
  describe('sendOTP', () => {
    ...
    it('văng lỗi nếu user đăng ký trùng Email', async () => {
      ...
      await expect(service.sendOTP({ email: 't@t.c', type: TypeOfVerificationCode.REGISTER })).rejects.toThrow(UnprocessableEntityException);
    });

    it('văng lỗi nếu gửi mã FORGOT cho email ko tồn tại', async () => {
      shareUserRepo.findUnique.mockResolvedValue(null); // Ko tồn tại
      await expect(service.sendOTP({ email: 't@t.c', type: TypeOfVerificationCode.FORGOT_PASSWORD })).rejects.toThrow(UnprocessableEntityException);
    });
  ...
  describe('login', () => {
    it('Đăng nhập thành công', async () => {
      authRepo.findUniqueIncludeRole.mockResolvedValue({
        id: 1, email: 't@t.c', password: 'hashed', roleId: 2, role: { name: 'CUSTOMER' }, totpSecret: null
      } as any);
    ...
    it('Báo lỗi sai email', async () => {
      ...
      await expect(service.login({ email: 't@t.c', password: 'password', ip: '1.2.3.4', userAgent: 'Chrome' }))
        .rejects.toThrow(UnprocessableEntityException);
    });
    ...
    it('Cần code do bật 2FA/totp', async () => {
      ...
      // Thiếu code -> throw
      await expect(service.login({ email: 't@t.c', password: 'password', ip: '1', userAgent: '1' }))
        .rejects.toThrow(UnprocessableEntityException);
    });
  });
  ...
  describe('refreshToken', () => {
    ...
    it('văng Unauthorized khi db k thấy RT cũ (tức là đã revoke)', async () => {
      ...
      await expect(service.refreshToken({ refreshToken: 'RT', ip: '1', userAgent: '1' })).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe('forgotPassword', () => {
    it('đổi mật khẩu thành công', async () => {
      ...
      const res = await service.forgotPassword({ email: 't@t.c', code: '123', newPassword: 'NEW', confirmNewPassword: 'NEW' });
    ...
    it('thất bại do email k tồn tại', async () => {
      shareUserRepo.findUnique.mockResolvedValue(null);
      await expect(service.forgotPassword({ email: 'nope', code: '', newPassword: '', confirmNewPassword: '' })).rejects.toThrow(UnprocessableEntityException);
    });
  });
});

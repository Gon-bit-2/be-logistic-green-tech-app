...
import { StripsModule } from 'src/modules/trips/trips.module';
import { OrdersController } from 'src/modules/orders/controller/orders.controller';
import { StripsController } from 'src/modules/trips/controller/trips.controller';
...
describe('AppModule', () => {
  it('mounts orders and trips modules in the application runtime', () => {
    ...
    expect(imports).toContain(StripsModule);
  });
  ...
  it('keeps orders and trips controller routes published', () => {
    ...
    expect(Reflect.getMetadata(PATH_METADATA, StripsController)).toBe('trips');
  });
});

...
async function main() {
  ...
  const hubs = await Promise.all([
    prisma.hub.upsert({ where: { code: 'HN-HUB-01' }, update: {}, create: { code: 'HN-HUB-01', name: 'Kho Trung Chuyển Hà Nội', address: '123 Nguyễn Trãi, Thanh Xuân, Hà Nội', latitude: 21.0024, longitude: 105.8041 } }),
    prisma.hub.upsert({ where: { code: 'DN-HUB-01' }, update: {}, create: { code: 'DN-HUB-01', name: 'Kho Trung Chuyển Đà Nẵng', address: '45 Điện Biên Phủ, Thanh Khê, Đà Nẵng', latitude: 16.0544, longitude: 108.2022 } }),
    prisma.hub.upsert({ where: { code: 'HCM-HUB-01' }, update: {}, create: { code: 'HCM-HUB-01', name: 'Kho Trung Chuyển TP.HCM', address: '789 Cách Mạng Tháng 8, Quận 3, TP.HCM', latitude: 10.7769, longitude: 106.6951 } }),
  ])
  console.log(`✅ Đã tạo ${hubs.length} kho trung chuyển`)

  // ===== SEED VEHICLES =====
  const vehicleData = [
    { licensePlate: '30A-11111', type: 'ELECTRIC_VAN' as const, fuelType: 'ELECTRIC' as const, capacityWeight: 1500, capacityVolume: 8.0, emissionRatePerKm: 0, hubId: hubs[0].id },
    { licensePlate: '30A-11113', type: 'TRUCK' as const, fuelType: 'DIESEL' as const, capacityWeight: 5000, capacityVolume: 25.0, emissionRatePerKm: 250, hubId: hubs[0].id },
    { licensePlate: '43A-22221', type: 'ELECTRIC_VAN' as const, fuelType: 'ELECTRIC' as const, capacityWeight: 1800, capacityVolume: 9.0, emissionRatePerKm: 0, hubId: hubs[1].id },
    { licensePlate: '51A-33331', type: 'ELECTRIC_VAN' as const, fuelType: 'ELECTRIC' as const, capacityWeight: 2500, capacityVolume: 12.0, emissionRatePerKm: 0, hubId: hubs[2].id },
  ]
  let vehicleCount = 0
    ...
    for(let i=1; i<=10; i++) {
        await prisma.order.create({
            data: {
                ...
                senderLng: 105.80 + (Math.random() * 0.01),
